...
select team,`rank`,`group`  from group_stage_team_stats
...
#6.Write an SQL query to show team, percent of wins with respect to matches_played by each team 
...
select team, round(100*(draws/matches_played),2) 
from group_stage_team_stats; 
...
#10.Write an SQL query to show percent of losses with respect to matches_played by each team
#in ascending order by losses and name the resulting column as losses_percen

select team , 100*(losses/matches_played ) as losses_percen from group_stage_team_stats
order by losses_percen asc;
...
select team ,points from group_stage_team_stats
where points=0;
...
#23.Write an SQL query to show team, cards_yellow, cards_red where cards_red is equal to 1 
...
#29.Write an SQL query to show team, errors, touches 
#for which errors is 0 and touches is less than 1500 
...
#31.Write an SQL query to show team, offsides which has offsides less than 10 
#or greater than 20
...
select count(team),`group` from group_stage_team_stats
group by `group`;

#34.Write a SQL query to show team names group 6 has

select team,`group` from group_stage_team_stats
where `group`=6;

#35.Write an SQL query to show Australia belongs to which group 

select team,`group` from group_stage_team_stats
where `group`= "Australia";
...
#37.Write an SQL query to show group, maximum expected_goal_scored 
#by each group in ascending order by expected_goal_scored

select `group` ,max(expected_goal_scored)  from 
group_stage_team_stats
group by `group`,expected_goal_scored
...
group by `group`,exp_goal_conceded 
...
group by `group`,exp_goal_difference_per_90
order by exp_goal_difference_per_90 asc;
...
#45.Write an SQL query to show average avg_age of each group 
#and arrange it in descending order by avg_age. 

select `group`,avg(avg_age) from 
group_stage_team_stats inner join overall_wc_stats
on group_stage_team_stats.team =overall_wc_stats.team
group by `group`,avg_age
order by avg_age desc;



#46.Write an SQL query to show sum of fouls for each group 
#and arrange it in ascending order by fouls.


select `group`,sum(fouls) from 
group_stage_team_stats inner join overall_wc_stats
on group_stage_team_stats.team = overall_wc_stats.team
group by `group`,fouls
order by fouls asc;


#47.Write an SQL query to show total number of games for each group 
#and arrange it in descending order by games. 
select `group`,sum(games) as total_games from 
group_stage_team_stats inner join overall_wc_stats
on group_stage_team_stats.team=overall_wc_stats.team
group by `group`
...
#48.Write an SQL query to show total number of players_used for each group 
#and arrange it in ascending order by players_used. 


select `group`,sum(players_used) as total_player from 
group_stage_team_stats inner join overall_wc_stats
on group_stage_team_stats.team = overall_wc_stats.team
group by `group` 
order by total_player asc;
...
select `group`, sum(offsides) as total_offsides from
group_stage_team_stats inner join overall_wc_stats
on group_stage_team_stats.team = overall_wc_stats.team
group by `group`
...
select `group`, avg(passes_pct) as avg_passes from 
group_stage_team_stats inner join  overall_wc_stats 
on group_stage_team_stats.team =overall_wc_stats.team
group by `group`
...
select `group` , avg(goals_per90) as avg_goals_per_90 from
group_stage_team_stats inner join overall_wc_stats 
on group_stage_team_stats.team= overall_wc_stats.team
group by `group`
order by avg_goals_per_90 asc;
